...
const Photography = () => {
  ...
  const allPhotos = [
    ...dbPhotos.map((p) => ({ title: p.title, slug: p.slug, location: p.location, year: p.year, image: p.image_url || "" })),
              ...
              {allPhotos.map((photo, i) => (
                <motion.div key={photo.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  ...
                  <div className="aspect-square overflow-hidden">
                    {photo.image ? <img src={photo.image} alt={photo.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
